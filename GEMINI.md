...
5.  **Phase 5: Review & Docs**: Invoke `documentation-architect` to review if the implementation matches the `New-Requirements.MD` list. If deviations are found, the `documentation-architect` must document them and report back to the Master Agent. The Master Agent will then decide to either approve the deviation or send it back to Phase 2 for correction. If compliant, update feature status and finalize documentation.
