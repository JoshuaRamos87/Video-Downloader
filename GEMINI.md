...
- **The "Search/Replace" & File Creation Rule:** Sub-agents must never pass back entire existing code files. Modifications MUST use targeted Search/Replace blocks (e.g., `<<<< SEARCH` ... `>>>> REPLACE`). Do NOT attempt standard Git `.patch` formatting. If creating a brand new file, the agent must output the full file contents using exact `<<<< NEW FILE: path/to/file.ext >>>>` and `<<<< END NEW FILE >>>>` boundaries.
...
2.  **Phase 2: Development**: Invoke `developer-pro` to implement the tasks outlined in `New-Requirements.MD`. 
    ...
    * **Dependencies:** If a new npm package is required, explicitly list the necessary `npm install <package>` command at the top of the response so the environment can be updated before proceeding.
